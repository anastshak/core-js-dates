...
function getNextFriday(date) {
  ...
  const addDaysNumber = currentDay < 5 ? 5 - currentDay : 12 - currentDay;
  const nextFriday = new Date(date.setDate(date.getDate() + addDaysNumber));
  return nextFriday;
}
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  ...
  const period = (new Date(dateEnd) - new Date(dateStart)) / msInDay;
  return Math.ceil(period) + 1;
}
...
function getCountWeekendsInMonth(month, year) {
  ...
  for (let i = 1; i <= daysInMonth; i += 1) {
    const thisDay = new Date(year, month - 1, i);
